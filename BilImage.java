...
class BilImage extends JFrame implements ActionListener
{
	...
	static {
		try{
			File file=new File("C:\\Users\\WIN8.1\\Downloads\\BilImage");
			if(!file.exists())
				file.mkdir();
...
class Fcm extends JFrame implements ActionListener
{
  ...
  Double varienceArray[];
  static double varience_Array[];
  static Vector avarageValue=new Vector();   //for storing average values
  Double avarageArray[];
  static double avarage_Array[];
  static double old_avg[][];
  double pixel[][];
	  ...
	  for(int i=0;i<20;i++)
  {
   ...
   // box4.addItemListener(this);
        sel_distance.addActionListener(this);
...
public double determinant(double a[][],int n)
{
	...
	return det;
      ...
      {
    	 int no_of_itr=0,no_of_clstr=0;
        double meu_mix_db=0;
	   	if(flag_second==0)
       meu_mix_db=Double.parseDouble(meu_mix.getText());

	   	
	   	String temp2= JOptionPane.showInputDialog("enter number of clusteers you want: ");
	    	 no_of_clstr=Integer.parseInt(temp2);
	   	
	    	 String temp3= JOptionPane.showInputDialog("enter number of iterations : ");
	    	 no_of_itr=Integer.parseInt(temp3);
	   ...
	   if (source==btClasi || source==btsyn)
       {
			...
			meu=new double[no_of_clstr+1][h*w];
                        meun=new double[b+1][h*w];
                        meunm=new double[count+1][h*w];
			dsq=new double[no_of_clstr+1][h*w];
                        xnew=new double[b+1][h*w];
                        dsqs=new double[count+1][h*w];
 		    x=new double[b];
 		    sub=new double[b][1];
 		    subT=new double[1][b];
  		    FileInputStream fin[]=new FileInputStream[b];
  		    FileChannel fchan[]=new FileChannel[b];
  		    mean=new double[no_of_clstr][b];
  		    mul=new double[1][b];
  		    MappedByteBuffer mBuf[]=new MappedByteBuffer[b];
  		    double c1[][][]=new double[no_of_clstr+1][b][b];
                     double rese[][]=new double[count+1][h*w];
  		   ...
  		   if(source==btClasi)
  		    ...
  		    if(classifier_chk[classi-1]=="Fuzzy C-Means") {
  		    	System.out.println("yipppppeeeeeeeeee");
  		    	 String temp= JOptionPane.showInputDialog("enter number of iterations: ");
  		    	 no_of_itr=Integer.parseInt(temp);
           ...
           try
			{
             ...
             //...............Different Measures of Similarity and Dissimilarity..............
                   ...
                   if(txt.compareTo("Euclidean1")==0)
					                       {
                	 //assume mean in initial iteration
                	   System.out.println("hoooooooooooooooooooo");
                	                                for(int k=0;k<b;k++)
                	                               {
                		                              mean[l][k]=Math.random()%255;
                	                                }
											         for(int k=0;k<b;k++)
													 ...
													 System.out.println("distance for "+l+" cluster and "+i+" pixel is"+dsq[l][i]);
...
//............................. SIN .............................
   ...
   //............................. TAN.............................
	...
	///********* Calculating Meu Array by Distance vector for Fuzzy C-Means *************////
                if(classi==1)
	   			{
	   				for(int j=0;j<no_of_clstr;j++) {
	   				for(int k=0;k<b;k++)
                    {
                       mean[j][k]=Math.random()*255;
                     }
	   				}
						

	   				for(int itr=0;itr<no_of_itr;itr++) {
		   			try
		  		    {
		  			  ...
		  			  for(int i=0;i<((h*w));i++)
						 {
							 for(int j=0;j<no_of_clstr;j++)
							 {
								 for(int k=0;k<b;k++)
										 {
											 dsq[j][i]+=Math.pow(xnew[k][i]-mean[j][k], 2);
										 }
								// dsq[j][i]=Math.sqrt(dsq[j][i]);
								 System.out.println("here distance for "+j+" cluster and "+i+" pixel is"+dsq[j][i]);
							 }
						 }
		  				...
		  				{
		  					div=0.0;
		  					for(int k=0;k<no_of_clstr;k++)
		  					{
		  						div=div+dsq[k][i];
		  						//div=div+ Math.pow((dsq[j][i]/dsq[k][i]),(1/(m-1)));
		  					}
                             div=Math.pow((dsq[j][i]/div),(1/(m-1)));
		  					//meu[j][i]=1/(div+Math.pow((dsq[j][i]/del),(1/(m-1))));
		  					meu[j][i]=1/(div);
		  					System.out.println("meu is calculated for "+j+" cluster and "+i+" pixel ="+meu[j][i]);
		  				    }
		  					System.gc();
		  				}
		  				int itr1=itr+1;
		   			System.out.println("meu calculated "+itr1+" times");
		  		    }
		  			...
		  			catch(Exception e)
		  			{
		   			...
		   			try {
		   				...
		   				System.out.println("Mean Calculation Error : "+exc);
		   			}
		   			for(int j=0;j<no_of_clstr;j++) {
		   				for(int k=0;k<b;k++)
	                    {
	                       System.out.print(mean[j][k]+" ");
	                     }
		   				System.out.print("\n");
		   				}
	   				}
                       ...
                       mean[j][k]=Math.random()%255;
						 ...
						 for(int i=0;i<h*w;i++)
						 {
							 for(int j=0;j<5;j++)
							 {
								 ...
								 System.out.println("distance for "+j+" cluster and "+i+" pixel is"+dsq[j][i]);
							 }
						 }
		  		...
		  		/*		for(int i=0;i<h*w;i++)
		   			 ...
		   			 for(int i=0;i<h*w;i++)
					 {
						 for(int n=0;n<5;n++)
						 {
							 for(int k=0;k<b;k++)
									 {
										 dsq[n][i]+=Math.pow(xnew[k][i]-mean[n][k], 2);
									 }
							 System.out.println("distance for "+n+" cluster and "+i+" pixel is"+dsq[n][i]);
						 }
					 }
		   				}
                  ...
                  for(int itr=0;itr<no_of_itr;itr++) {                						 
					  		try
					  		{
				  				...
				  				for(int i=0;i<(h*w);i++)
				  				{
		                             ...
		                             div=Math.pow((dsq[j][i]/div),(1/(m-1)));
				  					//meu[j][i]=1/(div+Math.pow((dsq[j][i]/del),(1/(m-1))));
				  					meu[j][i]=1/(div);
				  					System.out.println("iteration "+itr+" meu is calculated for "+j+" cluster and "+i+" pixel ="+meu[j][i]);
				  				    }
				  					System.gc();
				  				}
				  				int itr1=itr+1;
				   			...
				   			catch(Exception e)
				  			{
				   			...
				   			try
				   			{
                           		double val1=0.0;
					  			double val2=0.0;
					  			double []arrayEta=new double[no_of_clstr];


					  			for(int i=0;i<no_of_clstr;i++)
					  			{
					  				val1=0.0;
						  			for(int j=0;j<h*w;j++)
						  			{
	...
	System.gc();
	 ...
	 /////////////********** Classification Of Boundary Pixels ***********////////////////
		...
		// 		Fcm fcm1=new Fcm();
				...
				double meu1[][]=new double[count][h*w];
				double deviation[][]=new double[count][h*w];
				double ft[][]=new double[count][h*w];
				double ft2[][]=new double[count][h*w];
    		   ...
    		   try
     			{
	            	...
	            	{
						System.out.println("saving file"+name+j2);
